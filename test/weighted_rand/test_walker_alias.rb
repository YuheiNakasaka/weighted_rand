...
class TestWalkerAlias < Minitest::Test
  def test_it_return_random_value
    weighted_hash = { 'a' => 1, 'b' => 2, 'c' => 3, 'd' => 4, 'e' => 5 }
    weighted_rand = WeightedRand::WalkerAlias.new(weighted_hash)
  ...
  def test_it_raise_error_if_invalid_argument_is_passed
    assert_raises(WeightedRand::WrongArgumentError) { WeightedRand::WalkerAlias.new([1, 2, 3]) }
  end
end